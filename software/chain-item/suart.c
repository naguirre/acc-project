...
#define	TX_HIGH		(1<<COM0A1^1<<COM0A0)
#define	TX_LOW		(TX_HIGH^1<<COM0A0)
#define	TX_OUT		TCCR0A		// use compare output mode
...
{
  OCR0A = BIT_TIME - 1;
  TCCR0A = TX_HIGH;			// set OC1A high, T1 mode 4
  TCCR0B = 1<<ICNC0^1<<WGM02^1<<CS00;	// noise canceler, 1-0 transition,
					// CLK/1, T1 mode 4 (CTC)
  TCCR0C = 1<<FOC0A;
  stx_state = 0;
  stx_in = 0;
  ...
  TIFR0 = 1<<ICF0;			// clear pending interrupt
  TIMSK0 = 1<<ICIE0^1<<OCIE0A;		// enable tx and wait for start
}
...
ISR( TIM0_CAPT_vect )				// start detection
{
  s16 i = ICR0 - BIT_TIME / 2;			// scan at 0.5 bit time
...
ISR( TIM0_COMPB_vect )				// receive data bits
{
  u8 i;
  ...
  TIMSK0 = 1<<ICIE0^1<<OCIE0A;			// enable next start
}


ISR( TIM0_COMPA_vect )				// transmit data bits
{
  if( stx_state ){
    stx_state--;
    TX_OUT = TX_HIGH;
